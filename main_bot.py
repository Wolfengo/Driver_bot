...
def otmena():
    ...
    otmena = types.InlineKeyboardButton(text="otmena", callback_data='otmena')
    keyboard.add(otmena)
    return keyboard
...
def if_not_drive():
    keyboard = types.InlineKeyboardMarkup()
    reg_driver = types.InlineKeyboardButton(text="Регистрация водителем", callback_data='reg_driver')
    # reg_part = types.InlineKeyboardButton(text="Регистрация парком", callback_data="reg_part")
    keyboard.add(reg_driver)
    # keyboard.add(reg_part)
    return keyboard
...
def enter_chat_number(message):
    ...
    if len(number) != 0:
        ...
        if url.json()["status"] == "EXISTS":
            Work_in_db.add_car_doc(message.chat.id, 'in url data base')
            Work_in_db.add_drive_doc1(message.chat.id, 'in url data base')
            Work_in_db.add_drive_doc2(message.chat.id, 'in url data base')
...
def photo(message):
    try:
        if Work_in_db.user_exists_auto(message.from_user.id)[0][0] is None:
            ...
            bot.send_message(
                message.chat.id,
                f"Фото документа на машину получено")
            take_photo(message)
            ...
            bot.send_message(
                message.chat.id,
                f"Фото лицевой стороны В/У получено")
            take_photo(message)
            ...
            bot.send_message(
                message.chat.id,
                f"Фото обратной стороны В/У получено")
            take_photo(message)
        else:
            take_photo(message)
    except TypeError:
...
def take_photo(mess):
    if Work_in_db.user_exists_auto(mess.from_user.id)[0][0] is None:
        ...
        if Work_in_db.user_exists_auto(mess.from_user.id)[0][0] is None and \
                Work_in_db.user_exists_drive1(mess.from_user.id)[0][0] is None and \
                Work_in_db.user_exists_drive2(mess.from_user.id)[0][0] is None:
            bot.send_message(
                mess.from_user.id,
                f"Если ввели не тот номер, напишите 0!")
        ...
        if Work_in_db.user_exists_register(call.from_user.id)[0][0] == "404":
            ...
            if Work_in_db.user_exists_auto(call.from_user.id)[0][0] is not None and \
                ...
                bot.send_message(
                    ...
                    f"Я вас уведомлю после успешной регистрации и дам доступ к работе над "
                    f"документами\n"
                    f"По всем вопросам можете обратиться по номеру +375255301576")
            else:
                bot.delete_message(call.message.chat.id, call.message.message_id)
                bot.send_message(
                    call.message.chat.id,
                    f"Регистрируем вас как водителя! \n")
                take_photo(call)
